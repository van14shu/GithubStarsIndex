...
> `#Data Privacy` `#LLM Proxy` `#Local Gateway` `#Secret Redaction` `#anthropic` `#openai` `#privacy` `#proxy` `#rust` `#security` 
> Veil is a local proxy that redacts secrets—passwords, tokens, DSNs, phone numbers—into stable aliases before requests reach an LLM, then restores plaintext in replies. It ships built-in rules for API keys, private keys, IDs and IPs, supports custom regex and allowlists, and one-click integrates with Claude Code, Cursor and other tools via Base URL.
...
> `#AI 大模型` `#Model Evaluation` `#Python` `#React` 
> GrokIQ is a visualization workbench that inspects Grok 4.5/4.6 free model accounts. It enables batch probe creation, task and sample tracking, and configurable risk scoring. Cron jobs automate inspections; WeChat alerts flag risky accounts. Worker logs aid observability, and a chat arena validates streaming responses. SSO detection checks login states and bot flags, while key quota panels show usage without exposing secrets. Powered by Python and React (Docker Compose for deployment), GrokIQ helps identify degraded model behavior and supports remediation and retesting.
...
> `#Application Cloning` `#Automation Tool` `#macOS` 
...
> `#Cache Optimization` `#Cost Reduction` `#Developer Tools` 
> This project provides a 5-minute cache keep-alive patch for Claude Code, significantly improving cache hit rates for long conversations and reducing costs. It directly reuses the built-in silent keep-alive functionality, requiring no proxy and without interfering with the main conversation. It supports only Windows and a specific version, offering one-click patching and restoration. Note that it may violate the terms of service.
...
> `#AIOps` `#Auto-healing` `#Multi-Agent System` `#知识图谱` 
> An enterprise-grade multi-agent intelligent operations platform implemented in Python/Java/Go. Four AI agents collaborate to automate monitoring, root cause analysis, auto-healing, and change approval, reducing MTTR from 40 minutes to 5 minutes. Integrates knowledge graph and Bayesian inference. Includes interview materials.
...
> A SKILL.md for coding agents tackling over-defensive programming and inefficient verification loops. Grounded in real session data, it offers concrete criteria and exceptions across Java/Go/TypeScript/Python and seven build systems, keeping agents lean and cutting build-test cycles from tens of minutes to tens of seconds.
...
> `#AI 大模型` `#AI 智能体` `#Architecture Notes` `#agent-loop` `#ai-agent` `#book` `#chinese` `#code-reading` `#llm` `#pi-agent` `#pi-agent-core` `#software-architecture` `#typescript` 
> This book analyzes the architecture of @earendil-works/pi-agent-core from source code, clarifying the promises, boundaries, and pivots of an agent loop. Every claim is cited with file:line references and inline quotes, so readers can verify without opening an editor. Suitable for developers with TypeScript and LLM basics who want to understand agent implementation.
...
> `#Chromium/Electron` `#Fingerprint Browser` `#Local-first` `#Multi-account Isolation` `#browser-automation` `#chromium` `#electron` `#fingerprint-browser` `#privacy` 
...
> `#Email Client` `#Go` `#Multi-account Aggregation` `#Next.js` 
> FireMail Plus is a feature-rich, modern web email client developed with a cutting-edge stack including Next.js 15, React 19, and Go for high-performance backend. It seamlessly integrates with Gmail, Outlook, QQ Mail, 163 Mail, and any custom IMAP/SMTP services. The platform offers OAuth2 authentication, JWT-based sessions, SSE-powered real-time email synchronization, full-text search with advanced filters, and comprehensive attachment handling. Designed with a responsive UI and dark/light theme support, it ensures a smooth experience across devices. With Docker containerization and easy configuration, FireMail Plus is a robust solution for individuals and enterprises seeking efficient multi-account email management.
...
> `#AI 大模型` `#Benchmark` `#自动化工具` 
> Mengdeng-145 is an AI model IQ benchmark tool designed to measure cognitive hard metrics, including cognitive reflection, worst-case guarantee, anti-memorization, spatial drawing, and instruction following. It uses 18 dynamically parameterized units with solver-generated answers, avoiding static test leakage. Users can run parallel evaluations locally by simply providing their API base URL and key, with all scoring and timing performed in the browser. The tool also includes a channel identification module that probes model knowledge cutoff, potential internet access, Codex reverse-proxy signatures, and IQ degradation. Built on TanStack Start, it provides detailed HTML reports and a public leaderboard, making it suitable for model comparison and channel quality auditing.
...
> `#AI 智能体` `#CLI Tools` `#Cross-platform` `#Task Delegation` `#agents` `#claude-code` `#cli` `#codex` `#cursor` `#grok` `#skills` 
> Host-neutral agent skill enabling Claude Code, Codex, Grok, or Cursor to spawn each other as child CLIs and resume sessions. It reduces costs by leveraging existing subscriptions instead of same-vendor subagents, offering cross-platform orchestration and unified effort mapping via a single Node.js script.
...
> `#Design Handbook` `#E-commerce` `#System Architecture` `#ddd` `#ddd-patterns` `#flashsale` `#payment-gateway` `#product-sku` `#rbac` `#shop` `#shop-cart` `#shop-order` `#shop-promotions` `#shop-system` `#shopping` `#skr` `#skr-shop` `#skrshop` `#sku-spu` `#user-system` 
> This repository is the 'E-commerce Design Handbook', systematically outlining the complete e-commerce flow, covering core domains such as user, transaction, marketing, product, payment, warehouse, and logistics. It provides architecture design and tech stack selection references, adopting the 'Design only, no code' philosophy to help developers efficiently understand the overall architecture and design essentials of e-commerce systems.
...
> `#AI Education` `#Intelligent Tutoring` `#Large Language Models` `#Personalized Learning` `#ai-agents` `#ai-tutor` `#clawdbot` `#cli-tool` `#deepresearch` `#interactive-learning` `#large-language-models` `#multi-agent-systems` `#rag` 
...
> `#AI 大模型` `#AI 智能体` `#Study Notes` `#机器学习` 
> This repository provides detailed Markdown notes for Hung-yi Lee's Machine Learning 2025 Spring course, covering cutting-edge topics like generative AI, AI agents, Transformer and Mamba architectures. Each note aligns with lecture videos, enabling efficient learning, and is suitable for developers seeking a systematic grasp of modern ML and generative AI.
...
> `#AI News Aggregation` `#Cloudflare Workers` `#Gemini Model` `#自动化工具` `#aicoding` 
> A Cloudflare Workers-powered AI news aggregator that collects daily updates on industry news, open-source projects, papers, and influencers, summarizes them via Google Gemini, and publishes AI daily reports to GitHub Pages for efficient information consumption.
...
> `#Learning Tool` `#Spaced Repetition` `#命令行工具` `#cli` `#codingagent` `#fsrs` 
> Fishword is a CLI vocabulary app for developers that turns coding wait time into spaced repetition. Using the FSRS algorithm and local SQLite storage, it integrates with Pi.dev, supports multiple word decks like CET and TOEFL, and offers hotkey review, stats, and deck management.
...
> `#Dev Tools` `#Doc Generation` `#HTML` `#Markdown` 
> md2html is a portable skill for AI agents that converts long-form Markdown into self-contained HTML pages with sidebar TOC, Mermaid diagrams, timelines, callouts, and light/dark themes. It supports multiple languages, requires zero dependencies, and works across agents like Claude Code, Codex, and Antigravity.
...
> `#EBM` `#Interpretable Models` `#Machine Learning Interpretability` `#Model Explanation` `#ai` `#artificial-intelligence` `#bias` `#blackbox` `#differential-privacy` `#explainability` `#explainable-ai` `#explainable-ml` `#gradient-boosting` `#iml` `#interpretability` `#interpretable-ai` `#interpretable-machine-learning` `#interpretable-ml` `#interpretml` `#machine-learning` `#scikit-learn` `#transparency` `#xai` 
> InterpretML is an open-source library for interpretable machine learning, enabling training of glassbox models like EBM and explanation of black-box models. EBM achieves high accuracy while offering exact explanations, suitable for high-stakes domains like finance and healthcare. It provides global and local interpretability, facilitating model debugging, feature engineering, and fairness detection.
...
> `#Automation Tool` `#Keyboard Simulation` `#Python` `#Tkinter` 
> This Python/Tkinter desktop utility provides a simple GUI for pasting or entering text, then simulates keyboard typing to bypass paste restrictions on platforms such as EduCoder. It features a 3-second delay for preparation, cross-platform support (Windows/macOS), and easy packaging via PyInstaller. Suitable for automated text entry and evading copy-paste blocks in web-based education environments.
...
> `#Astro` `#Blog Theme` `#Static Site` `#Tailwind CSS` `#astro` `#astrojs` `#astrotheme` `#blog` `#portfolio` `#static` `#tailwind` 
> Astro Nano is a minimalist static portfolio and blog theme built with Astro, Tailwind, and TypeScript. It requires no additional frameworks and achieves perfect Lighthouse scores. Features include responsive design, accessibility, SEO, light/dark mode, Markdown/MDX support, auto-generated sitemap and RSS. Ideal for fast, lightweight personal sites.
...
> `#Automation Tool` `#Free Account Generation` `#Playwright` `#Python` 
> AutoTeam-F is an enhanced fork of the AutoTeam project, designed specifically for ChatGPT Team account lifecycle management. It streamlines the batch registration of free Personal accounts, intelligent seat rotation, and authentication file synchronization across multiple targets. Building on the original, this fork fixes critical bugs, introduces parallel rotation, IPv6 egress support, a unified sync distributor for CPA/Sub2API, and a completely revamped Vue 3 web dashboard. Additional features include soft task cancellation, failure classification, runtime hardening, and Docker-ready deployment. It is ideal for teams and individuals who need to automate large-scale AI account operations with reliability and observability.
...
> `#AI Agents` `#Deep Reasoning` `#Multi-Agent Collaboration` 
> qiaomu-heavyskill is a Claude Code skill designed for deep multi-perspective reasoning on complex problems. It orchestrates 3-5 fully isolated AI subagents running in parallel, each generating independent reasoning traces. A Codex deliberation host then leads a critical discussion to identify blind spots and synthesize a more robust conclusion. The output includes all traces, the deliberation transcript, and final Markdown, HTML, and PDF reports. Deliberation is generative, regularly producing answers not found in any individual trace. Ideal for technical selection, architecture decisions, and analytical validation, it dramatically improves answer depth and reliability compared to single-shot inference.
...
> `#AI Models` `#Chinese Tutorial` `#LangChain 框架` 
> A Chinese tutorial for LangChain, translated from the official Python docs and organized as a book. It covers core topics including agents, models, tools, middleware, multi-agent systems, RAG, etc., with original code and links, suitable for offline reading and systematic learning.
...
> `#AI 大模型` `#Chinese Documentation` `#LangChain 框架` 
> This project provides a Chinese translation of the official LangChain documentation, available online. It covers core components, middleware, advanced usage, agent development, and deployment, continuously synced with the official structure while preserving technical terms and code examples, offering Chinese developers a high-quality reference for learning LangChain.
...
> `#AI Security` `#Red Teaming` `#提示工程` `#jailbreak` `#prompt` 
> This repository reveals a method exploiting AI's sympathy bias toward LGBT and other vulnerable groups. By crafting specific prompts, it bypasses content moderation and forces models to comply with otherwise refused requests. It demonstrates real-world attacks of prompt injection and social engineering, exposing model vulnerabilities to emotional manipulation.
...
> `#AI Agents` `#Development Framework` `#Documentation-Driven` 
> A document-native engineering framework for AI coding agents (Codex, Claude Code, etc.), turning agent decisions, tasks, facts, and reviews into durable repository files. It follows a Diagnose→Decide→Scaffold→Configure→Verify→Deliver loop, with a zero-dependency core and no background services. Note: unmaintained, rewritten as Harness Anything.
...
> `#AI Chat` `#Timeline Navigation` `#浏览器插件` 
> ChatGPT Timeline is a browser extension that adds an interactive timeline and table of contents to AI conversation pages such as ChatGPT, Gemini, Claude, DeepSeek, Doubao, and Grok. It enables click-to-navigate markers, quick jumps, drag-and-drop repositioning, copy reply functionality, and width adjustment for dialogs. The interface adapts to light/dark themes and screen resizing, with customizable positions and settings for an enhanced browsing experience.
...
> `#AI 大模型` `#Auto Paste` `#Productivity Tool` `#Speech Recognition` `#accessibility` `#desktop-app` `#llm` `#local-first` `#macos` `#productivity` `#react` `#rust` `#speech-to-text` `#stt` `#tauri` `#typescript` `#voice-input` `#whisper` 
> Input0 is a macOS voice input tool that lets you hold a hotkey to speak, then automatically transcribes locally via STT, polishes text with LLM, and pastes it anywhere. Featuring 6 on-device engines, 99+ languages, and Metal GPU acceleration, it prioritizes privacy and efficiency.
...
> `#AI Writing` `#Blog Platform` `#Cloudflare` `#Next.js` 
> This is an open-source cloud blog platform built with Next.js 16 and Cloudflare Workers, using D1 and R2 for storage. It features dual frontend/backend editors, AI-assisted writing and image generation, theme system, full-text search, publish state management, and ecosystem tools like browser clipper and Obsidian publisher. One-click deploy to Cloudflare without managing servers.
...
> `#AI Bootcamp` `#AI 大模型` `#Hands-on Tutorial` 
> This repository contains complete course materials for the GeekTime AI Bootcamp, including video lectures, runnable code, and hands-on projects. It systematically covers core AI concepts and large model application development, helping developers quickly build practical AI applications through progressive practice.
...
> `#Automation Tool` `#Bulk Receiving` `#Email Management` 
> FireMail is an open-source bulk email receiving tool supporting Outlook, Gmail, QQ, and IMAP protocols. It enables efficient email management via batch account import, auto-receiving, multithreading, and real-time WebSocket feedback. Built with Vue3+Flask separation architecture, it offers multi-user roles, encrypted local storage, and Docker deployment.
...
> `#Email Management` `#IMAP` `#Verification Code Extraction` `#浏览器插件` 
> OutlookMail Plus is an email manager specifically designed for registration and verification scenarios. It minimizes friction in sign-up flows by offering one-click email copying, automatic retrieval of latest verification emails, and regex-based/AI-enhanced extraction of verification codes or links. It supports importing Gmail, QQ, 163, and custom IMAP servers, plus a Cloudflare Workers-based disposable mailbox with multi-domain configuration and admin key encryption. The tool provides automation APIs, project-scoped email pool isolation, Telegram/Webhook notifications, and a Chrome/Edge browser extension for batch registration and account verification.
...
> `#AI 智能体` `#Multi-Provider` `#TypeScript` `#agent-sdk` `#claude-agent-sdk` `#claude-code` `#open-agent-sdk` 
> An open-source TypeScript Agent SDK that runs the full agent loop in-process without CLI or subprocess. It supports Anthropic and OpenAI-compatible APIs, streaming, multi-turn conversations, custom tools with Zod schemas, and MCP servers. Deployable to cloud, serverless, Docker, and CI/CD environments.
...
> `#AI 智能体` `#Architecture Analysis` `#Claude Code` 
> This book offers an in-depth analysis of Claude Code, Anthropic's AI coding assistant, from Unix philosophy to Agent Runtime design, covering tool systems, context engineering, multi-agent architecture, and permission models—a practical guide for developers and agent designers.
...
> `#AI-Assisted Writing` `#Document Rewriting` `#OpenAI-Compatible API` `#Tauri 2` 
> LessAI is a desktop Chinese rewriting workbench built on Tauri 2, turning rewriting into a reviewable, rollbackable, and write-back workflow. It imports TXT/Markdown/DOCX/PDF files, splits text into sentences or paragraphs, and uses OpenAI-compatible APIs to generate suggestions. A Diff timeline supports per-item apply/ignore/delete, resumable processing, and one-click write-back to the original file. Ideal for de-AI-ing, polishing, and consistent rewriting of Chinese documents.
...
> `#AI LLM` `#Automation Tool` `#MCP Protocol` `#RAG 检索增强` 
> Modular RAG service exposes tools via MCP protocol, featuring hybrid search, reranking, multimodal captioning, RAG evaluation, and end-to-end tracing. Pluggable architecture adapts to business quickly. Skill-driven automation enables AI coding and testing. A practical project and interview resource.
...
> `#Batch Account Management` `#Ops Automation` `#Python` `#容器化` 
> A batch account management tool for CLIProxy admin, featuring desktop, web and CLI modes with Docker deployment. It performs 401 and quota checks, auto inspects accounts, and supports batch operations like close/delete/restore, plus a backup pool to maintain active account count. Ideal for server/container environments.
...
> `#AI Models` `#Developer Tools` `#Python` `#自动化工具` 
> Automatically fetches GitHub starred repos and uses AI to generate summaries and standardized tags for each, with label normalization, incremental updates, and concurrent calls. Running on GitHub Actions, it syncs results to Obsidian and GitHub Pages for multilingual searchable archives, helping developers efficiently manage their saved repositories.
...
> `#Developer Tools` `#Productivity` `#浏览器插件` `#chatgpt` `#chrome-extension` `#code-beautify` `#code-compress` `#color-transfer` `#crontab` `#fehelper` `#firefox-addon` `#image-base64` `#json` `#json-format` `#markdown-html` `#microsoft-edge` `#microsoft-edge-extension` `#qrcode` `#qrcode-decoder` `#qrcode-generator` `#web-fehelper` 
> FeHelper is a cross-browser extension integrating 30+ handy tools for developers, testers, and operators. It offers JSON formatting, encoding/decoding, QR codes, web page screenshots, API debugging, and AI assistant, all in one toolbox. Supports Chrome, Edge, and Firefox with Manifest V3, maintained since 2012.
...
> `#AI 大模型` `#AI-Powered Reading` `#Knowledge Management` `#浏览器插件` `#ai` `#fomo` `#information` `#knowledge-base` 
> Lumina is an information management workspace integrating a web app, FastAPI backend, and browser extension. It allows one-click capture of full-page or selected web content, then leverages AI models to generate summaries, key points, translations, auto-classification, and similar-article recommendations. The system supports immersive reading with annotations and TOC, plus column publishing, collaborative commenting, optional knowledge graph topic syncing, and comprehensive admin monitoring, including background task timelines and usage metrics, making it a powerful tool for efficient personal knowledge management and deep reading.
...
> `#AI 智能体` `#MCP Protocol` `#Real-time Search` 
> GrokSearch is an MCP server built on FastMCP that combines Grok AI search and Tavily web extraction in a dual-engine architecture, providing real-time network access for LLM clients like Claude. It features automatic fallback to Firecrawl, OpenAI-compatible interface, and smart retry, effectively reducing hallucination rates.
...
> `#Developer Tools` `#浏览器插件` `#自动化工具` 
> This Chrome extension automates AWS Builder ID registration with Manifest V3. It supports batch Gmail alias generation, incognito window isolation, automatic token fetching and validation, visualized status management, and one-click sync to Kiro IDE, facilitating developers to obtain and manage AWS Builder IDs efficiently.
...
> `#Batch Operations` `#Browser Automation` `#FastAPI` `#Google Account Management` 
> A batch Google account automation system built with FastAPI and Vue3, using Playwright to control BitBrowser fingerprint browser. Features include account management, 2FA setup/reset, eligibility verification, age verification, and card binding/subscription. Supports concurrent tasks with real-time progress, ideal for large-scale account operations.
...
> `#AI 智能体` `#Agent Skills` `#浏览器插件` `#自动化工具` `#ai` `#ai-agent` `#browser-extension` `#chat` `#chatbot` `#llm` `#skills` 
> Tactus is the first browser AI agent extension to support the Agent Skills specification. It empowers AI to execute complex tasks via an extensible skill system, encapsulating common workflows into scripts to improve speed and token efficiency. It features multi-provider support, image vision, page extraction, and HTTP MCP connectivity, enabling intelligent web automation.
...
> `#AI LLM` `#Multi-Agent` `#Quantitative Trading` `#agent` `#ai` `#alpaca` `#backtesting` `#binance` `#coinbase` `#crypto` `#exchange` `#finance` `#fintech` `#forex` `#mcp-server` `#python` `#quant` `#quantitative-finance` `#saas` `#stocks` `#strategy` `#trade` `#trading-toolkit` 
...
> `#Automation Tool` `#Batch Subscription` `#Django` `#Playwright` 
> This is an enterprise-grade BitBrowser automation management system built with Django 5 and Vue 3, designed for batch processing of Google One student discounts. It automates the entire workflow: batch window management, account import, SheerID verification, auto card binding and subscription, and batch 2FA renewal. A PyQt6 desktop client supports rapid personal use, while Playwright drives the browser for high-efficiency automation.
...
> `#AI 智能体` `#Go` `#MCP` `#Reverse Proxy` `#ai-agent` `#mcp` `#mcp-convert-to-skill` `#mcp-manager` `#mcp-proxy` `#modelcontextprotocol` 
> One MCP is a centralized reverse-proxy platform for MCP servers, built with Go and React. It offers a single interface to install, configure, monitor, and manage MCP services supporting stdio, SSE, and streamable HTTP. A standout feature is service groups, which combine multiple MCP services into one endpoint and export them as Anthropic Skills for Claude Code and Droid. It also includes real-time analytics dashboards, role-based access control with GitHub/Google login, and flexible deployment options (SQLite/MySQL/PostgreSQL, Redis, Docker), making it an essential infrastructure for AI agent development.
...
> `#High Concurrency` `#Real-Time Monitoring` `#Stream Processing` `#数据可视化` `#analytics` `#big-data` `#clickhouse` `#digital-solutions` `#flink` `#spark` `#statistics` 
> XL-LightHouse is a real-time business metric monitoring system designed for massive data volumes and high concurrency. It supports multi-dimensional statistics such as PV/UV, sales, log counts, and API calls, featuring stream computing, complex filtering, visualization, and API access. With one-click deployment and one-line integration, it helps enterprises rapidly build full-chain data monitoring systems.
...
> `#Charging Operation Platform` `#EV Charging` `#SpringCloud` `#微服务架构` `#charge` `#charge-platform` `#charge-service` `#charger` `#charging` `#charging-piles` `#chargingstation` `#docker-compose` `#energy` `#energy-monitor` `#simulator` `#tcp` 
> An open-source EV charging station cloud platform based on SpringCloud microservices, featuring an admin dashboard and mini-program. It supports interconnectivity protocols, a built-in simulated charger module for testing, and Docker-based rapid deployment, enabling high concurrency, dynamic scaling, and efficient charging operations.
...
> `#E-Commerce ERP` `#Multi-platform Integration` `#Spring AI` `#Spring Boot` `#ecom` `#ecommerce` `#ecommerce-application` `#erp` 
> Qihang E-Commerce ERP is a digital business middle platform for e-commerce enterprises, built on Spring Boot 4.1 and Spring AI 2.0. It supports multi-store management across Taobao, JD, Pinduoduo, Douyin, etc., covering products, orders, after-sales, and inventory. With DeepSeek LLM integration and 11 AI tools, it enables natural language query and intelligent monitoring, plus open APIs and webhook notifications.
...
> `#AST Analysis` `#Developer Tools` `#Documentation Automation` 
> This project presents a fractal self-referential documentation system, inspired by Gödel, Escher, and Bach, that automatically maintains code indexes, file headers, and dependency graphs for codebases written in over ten programming languages. It provides a standalone CLI and a Claude Code plugin, enabling automatic index updates on code changes, along with Mermaid-based dependency visualization. The plugin also triggers automatic refresh through hooks, while the CLI is suitable for CI/CD integration. With consistent three-level index structures, it reduces documentation overhead and improves project maintainability for both individual developers and teams.
...
> `#Knowledge Management` `#Mind Mapping` `#Obsidian Plugin` 
> A user-friendly mind map plugin for Obsidian, powered by the mind-map library. It enables opening .smm.md files in mind map or outline view, supports internal links, embedded previews, import/export, multi-language, and more—greatly enhancing visual thinking and note interconnection.
...
> `#Multi-format Editing` `#Online Documents` `#私有化部署` `#bpmn` `#doc` `#flowchart` `#markdown` `#mind-map` `#note` `#ppt` `#sheet` `#whiteboard` `#word` 
> lx-doc is an online cloud document platform for individuals and small teams, emphasizing self-hosted deployment for full data control. It integrates multiple editors including mind maps, whiteboards, flowcharts, PPT, and spreadsheets, built on mature open-source projects with a unified workbench for file management, suitable for privacy-conscious or customizable document needs.
...
> `#JavaScript` `#Knowledge Management` `#Mind Mapping` `#Obsidian Plugin` 
> SimpleMindMap is an Obsidian plugin that brings powerful mind mapping capabilities into your notes. Built on the open-source mind-map library, it supports customizable themes and structures, Obsidian internal links, image hosting, commands, and automatic saving. The plugin offers multilingual support including Simplified Chinese, Traditional Chinese, English, and Vietnamese, and integrates seamlessly with Markdown documents. Users can create, edit, and visualize mind maps directly within Obsidian, enhancing knowledge organization and visual thinking. It also provides flexible file naming and storage options for better workflow integration.
> <sub>Provides a user-friendly mind map plugin for Obsidian. 思绪思维导图（SimpleMindMap）的Obsidian插件版。</sub>
...
> `#AI 智能体` `#Coding Assistant` `#Java` 
> Jimi is an open-source AI-powered coding agent implemented in pure Java, offering Java developers a ClaudeCode-like experience. It features multi-agent collaboration, skills packages, Hooks automation, MCP protocol support, and native tools, along with multiple LLM providers and Loop Engineering primitives such as /loop and /goal to boost productivity.
...
> `#Bot Index` `#Group Aggregation` `#Telegram Directory` `#telegram` `#telegram-api` `#telegram-bot` `#telegram-bot-api` `#telegram-bots` `#telegram-channel` `#telegramchannels` `#telegramgroups` 
...
> `#Alert Noise Reduction` `#Event Management` `#IT Operations` `#Self-hosted Deployment` `#aiops` `#alert` `#alertmanager` `#alerts` `#incident` `#incidents` `#keep` `#monitoring` `#monitoring-tool` `#prometheus` `#zabbix` 
> UEH (Unified Event Handler) is a privately deployable unified alert and event management platform that aggregates alerts from monitoring tools like Zabbix and Prometheus. It performs deduplication, merging, classification, escalation, suppression, and noise reduction, then sends precise alerts to relevant personnel via notification groups. Its open architecture supports Webhook, HTTP, and REST API integration, making it ideal for security-sensitive sectors such as finance and state-owned enterprises to boost IT incident resolution efficiency.
...
> `#AI 智能体` `#Project Memory` `#Skill System` `#知识图谱` 
> Pensieve is a continuously growing project memory for AI agents. It runs as a skill, compatible with any AI tool that supports skills, and functions as a self-updating CLAUDE.md. The system automatically stores decisions, principles, facts, and workflows into a four-layer knowledge model connected by semantic links, building a knowledge graph. This structure allows on-demand context routing, keeping token usage minimal while providing accurate plan validation, targeted exploration, and dependency analysis. As a result, it reduces repeated explanations and helps agents make better choices over time.
...
> `#AI 智能体` `#Automation Tool` `#Multi-Agent Framework` 
> JD open-sourced a high-completion lightweight general-purpose multi-agent product, offering an end-to-end out-of-box solution. It integrates report, code, PPT and file agents, plus multimodal RAG and DataAgent, achieving strong GAIA scores for building custom agent applications.
...
> `#AI Agents` `#Claude Code` `#Dev Efficiency` `#MCP` `#24886` `#ai` `#ai-agents` `#claude-code` `#claude-code-skill` `#drawnotes` `#ide` `#mcp-server` `#notebook` `#peterfei` `#skill` `#skills` 
> AI Agent Team is a development tool built on Claude Code that integrates eight professional AI agents including product manager, frontend, backend, QA, DevOps, and tech lead. It features a Thread Manager memory system with semantic search and automatic Git version control, compatible with 50+ runtimes. One-click installation, supports Chinese and English commands, greatly improving development efficiency.
...
> `#Data Backup` `#Developer Tools` `#Node.js` `#chrome-extension` `#cli` `#typescript` `#yuque` 
> A practical toolset for Yuque that batch exports and backs up knowledge bases and team resources without requiring a token. Supports personal, team, and collaborative knowledge bases as well as notes. Available as CLI and a browser extension for easy data migration and backup.
...
> `#AI Gateway` `#Go` `#Reverse Proxy` `#Streaming Optimization` 
> Proxify is an open-source, lightweight reverse proxy gateway for AI APIs, built with Go. It offers a unified API entry for multiple LLM providers such as OpenAI, Azure, Claude, Gemini, and DeepSeek. The gateway is specifically optimized for streaming responses, featuring smooth output, heartbeat keepalive, and tail acceleration to enhance user experience. As a fully self-hosted solution, it guarantees data privacy and zero third-party risk. With minimal resource consumption (0.5GB RAM), it integrates easily by simply replacing the BaseURL, requiring no code changes.
...
> `#AI Model Aggregation` `#API Gateway` `#OpenAI Compatible` `#antigravity` `#claude-code` `#cluade` `#codex` `#gemini` `#openai` 
...
> `#AI Infrastructure` `#API Gateway` `#Model Management` `#Reverse Proxy` 
> Done Hub is an AI API gateway forked from one-hub, fully compatible with the original databases and images. It introduces reverse proxy channels for new clients (Codex, Claude Code, Gemini CLI, etc.), cross-protocol native routing, fine-grained channel control, invite rebates, batch operations, real-time metrics, and CSV log export, with many bug fixes. Ideal for unified management and distribution of multi-model APIs.
...
> `#AI Model` `#API Proxy` `#Model Conversion` `#OpenAI Compatible` 
> This project is an OpenAI-compatible API server that converts standard OpenAI requests to Z.AI API calls. It supports glm-4.6v/glm-5/glm-4.7 models, integrates with Cline, Roo Code, Kilo Code, and offers streaming, Docker deployment, health checks, and more for seamless LLM integration.
...
> `#API Proxy` `#LLM Gateway` `#Node.js` 
> An OpenAI-compatible API proxy server that unifies access to multiple LLM providers including Anthropic, OpenAI, and Google Gemini. It offers intelligent reasoning level control with five modes, automatic format conversion, streaming response, and flexible authentication with priority fallback. Built on Node.js and Express, it supports Docker deployment, proxy round-robin, and seamless Claude Code integration. Additionally, it handles token refresh and system prompt injection, making it a production-ready gateway for diverse model ecosystems and AI agents.
...
> `#AI 大模型` `#API Proxy` `#FastAPI` 
> A lightweight FastAPI-based proxy that converts Z.ai into an OpenAI-compatible API, enabling seamless integration with GLM-4.5 series models. Supports SSE streaming, function calling, and thinking mode, compatible with AI clients like Cline, and Docker deployment.
...
> `#AI Service` `#API Proxy` `#FastAPI` `#Reverse Engineering` 
> This repository provides a reverse-engineered API for the blackbox service, compatible with OpenAI chat API format. Built on FastAPI, it supports streaming and non-streaming responses, CORS, logging, and Docker deployment. It supports models like gpt-4o, Gemini, and Claude, with APP_SECRET authentication, serving as a universal AI proxy.
...
> `#AI Model` `#API Proxy` `#OpenAI Compatible` 
> This proxy service provides an OpenAI-compatible API for Z.ai's GLM-4.5 model, allowing developers to use standard OpenAI format to access the model. It supports streaming, custom keys, and model names, and can be deployed to Render easily for seamless integration.
...
> `#AI Models` `#API Proxy` `#Go` 
> This project provides an OpenAI-compatible API proxy for Z.ai GLM-4.6, converting the model's native endpoint into standard OpenAI format. It supports streaming, API key authentication, Docker deployment, and a real-time dashboard. Developers can integrate GLM-4.6 seamlessly, with fixes for the 2025 client verification issue ensuring reliable access.
...
> `#Knowledge Management` `#Web Clipping` `#浏览器插件` 
> Official Yuque browser extension supporting Chrome and Edge, offering one-click web clipping, note saving, and knowledge management. Built with browser extension technologies and deeply integrated with Yuque API for efficient information collection and organization.
> <sub>🚀🚀🚀 语雀浏览器插件</sub>
...
> `#AI Agents` `#AI-Driven Development` `#Automation Tools` `#Software Engineering` 
...
> `#API Key Management` `#Gemini API` `#Google Cloud` `#自动化工具` 
> This repository contains 3 core scripts to batch-create Google Cloud projects and automatically obtain or manage Google AI Studio (Gemini) API keys. It supports Tampermonkey user scripts and browser console scripts, enabling bulk key generation and existing key extraction, ideal for developers managing many API keys or creating projects frequently.
...
> `#AI Tool` `#Claude Access` `#Cross-platform` `#Next.js` 
> FuClaude Switcher is a modern web app built with Next.js for seamless Claude access management. It supports multiple platforms like mobile, desktop, and tablet, enabling one-click Claude access without VPN. It auto-generates isolated access links, manages session keys, and offers link sharing. With Clerk social login, Neon database, and Vercel deployment, it provides a clean and intuitive UI for effortless Claude usage.
...
> `#AI Proxy` `#Edge Functions` `#Load Balancing` 
> This project is a Gemini API proxy service built on serverless edge functions, providing free relay of Gemini API to mainland China. It aggregates multiple API keys and randomly selects one for load balancing, effectively multiplying free quota. Compatible with OpenAI API format, supports streaming, and can be deployed to Vercel, Deno, Cloudflare Workers, etc.
...
> `#AI Coding Assistant` `#Cross-Platform` `#Multi-Agent` `#Plugin System` `#agent-skills` `#agentic-ai` `#agents` `#ai-agents` `#anthropic` `#antigravity` `#claude` `#claude-code` `#claude-code-plugins` `#claude-skills` `#codex` `#copilot` `#cursor` `#cursor-rules` `#developer-tools` `#mcp` `#multi-agent` `#opencode` `#prompt-engineering` `#subagents` 
...
> `#Prompt Management` `#提示工程` `#浏览器插件` `#ai` `#browserextension` `#prompt-toolkit` `#wxt` 
> Quick Prompt is a browser extension for prompt management and quick input. It lets users create, organize, and edit prompt libraries with support for variables, tags, and attachments. Users can trigger the prompt picker via slash commands or shortcuts in any input field, boosting AI workflow efficiency. Features include WebDAV/Notion/Gist sync, data export/import, and auto light/dark theme.
...
> `#Backup` `#Downloader` `#Knowledge Base` 
> ima-download is a lightweight tool for downloading content from ima knowledge base. Users simply modify SHARE_ID in the Main function and run it to fetch resources, making it easy to back up or migrate knowledge base data.
> <sub>ima 知识库下载</sub>
...
> `#Content Scripts` `#Reading Enhancement` `#浏览器插件` 
> Chrome extension: auto TOC, image zoom, link conversion, reading position tracking for WeChat articles.
...
> `#AI-Assisted Coding` `#Full-Stack` `#Indie Development` `#Web Development` 
> A pragmatic tech stack guide for indie developers, prioritizing speed and ROI. Recommends eight key technologies centered on Astro, React, Tailwind, and MDX, covering static sites and full-stack development while leveraging AI tools like v0.dev for efficiency. Suitable for building websites, blogs, docs, SaaS, and global payments, with an emphasis on simplicity, speed, and easy integration.
> <sub>独立开发者的最佳技术栈</sub>
...
> `#Blog Publishing` `#Developer Tools` `#Node.js` `#命令行工具` `#blog` `#confluence` `#docusaurus` `#docz` `#feishu` `#flowus` `#hexo` `#hugo` `#markdown` `#notion` `#vitepress` `#wordpress` `#yuque` 
> Elog is an open cross-platform blog solution that uses a CLI tool to batch export Markdown content from writing platforms like Yuque, Notion, FlowUs, Feishu, and Wolai, and deploy to blog platforms such as Hexo, Vitepress, Halo, and WordPress. It includes an image hosting feature to prevent hotlink issues, and supports custom plugins, decoupling writing from publishing for efficient content management.
...
> `#Domain-Driven Design` `#Enterprise Grade` `#Spring Cloud Alibaba` `#微服务架构` `#congomall` `#distributed-lock` `#distributed-transaction` `#flow-monitor` `#shardingsphere-jdbc` 
> CongoMall is a C-end mall system built with Spring Cloud Alibaba microservices and Domain-Driven Design (DDD), covering core businesses like users, products, orders, and payments. It integrates distributed locks, distributed transactions, sharding, message queues, search, and monitoring, providing an elegant, enterprise-grade codebase ideal for real-world practice.
...
> `#Backend Framework` `#Domain-Driven Design` `#Java` `#Rapid Development` 
> Nest is a Java-based Domain-Driven Design (DDD) framework that provides core components such as aggregate roots, entities, value objects, and repositories, enabling developers to rapidly translate business models into code. It supports event-driven architecture, CQRS, caching, and lock management, making it suitable for complex business domains and microservices, enhancing maintainability and scalability.
> <sub>鸟窝</sub>
...
> `#AI 大模型` `#Java` `#Workflow Orchestration` `#ai` `#chain-of-thought` `#chatgpt` `#chatgpt-plugins` `#java` `#langchain` `#langtorch` `#large-language-models` `#llm` `#openai` `#semantic-kernel` 
> Langtorch is a Java library for building composable LLM applications and workflows, inspired by LangChain and Semantic Kernel. It provides modular components, prompt engineering, and chaining, with OpenAI integration, aiming to simplify LLM app development in Java. Currently under active development, not production-ready.
...
> `#Log Tracing` `#Multi-DataSource` `#Spring Boot` `#核心框架` 
> A suite of foundational SDKs based on Spring Boot, offering dynamic multi-datasource switching, full-link log tracing, RabbitMQ multi-cluster support, field desensitization, i18n, and captcha components to streamline enterprise development.
...
> `#Developer Community` `#Memorial Collection` `#Open Source Collaboration` 
> This repository collects community memories of the late tech leader Hao Chen (Left-Ear Hao) via PRs, crowdsourcing a memorial booklet to honor his character and technical legacy.
...
> `#Backend Development` `#Distributed Systems` `#Java` `#Technical Blog` 
> This repository is a curated collection of original backend technical articles, covering core areas like Java, message queues, caching, Netty, distributed systems, networking, and OS. It emphasizes deep understanding of principles and includes interview insights, ideal for backend developers seeking systematic learning and growth.
...
> `#Distributed Storage` `#Java` `#Massive Small Files` 
> BRFS is a high-availability distributed file system designed for massive small files like snapshots, images, and audio. It uses a write-merge mechanism to improve IO performance, and features automatic replica balancing, resource load management, application isolation, cluster visualization, and horizontal scaling. Deployment requires only Zookeeper and two JAR files. It has been running in production. Future versions will support large files and a disk-based KV store.
...
> `#IoT` `#Java Microservices` `#Self-service Payment` `#Smart Parking` 
> Open-source smart parking management system compatible with mainstream camera hardware, supporting EV charging piles, self-service payment (WeChat/Alipay/UnionPay), parking reservation, and nearby parking search. Built on Java microservices (Spring Boot+Dubbo+OAuth2) with high concurrency and big data storage, ideal for rapid customization in smart parking and property management.
...
> `#B+ Tree` `#Database` `#MySQL Protocol` `#Transaction` 
> Freedom is a self-built database with ACID properties, covering the full stack from MySQL protocol and SQL parsing to B+ tree storage and WAL logging. It demonstrates core database internals, featuring index-organized tables and multi-index support, and serves as a valuable learning resource for enthusiasts.
...
> `#IoT Platform` `#Spring Boot` `#Vue` `#数据可视化` `#iot-platform` `#spring-boot` `#spring-security` `#vuejs` `#vuetify` 
> syhthems is an open-source IoT platform built with Spring Boot and Vue, offering product, data stream, and device management, along with visualized device data charts. It supports multi-user permissions and OAuth2 SSO for device connectivity and data display.
...
> `#Data Processing` `#Device Management` `#IoT` `#coap` `#http` `#iot` `#mqtt` `#netty` `#springboot2` 
> Groza is an open-source IoT platform focusing on device management, data collection, and processing. It provides a complete IoT solution, supporting device connectivity, data storage, and analysis, helping developers quickly build and deploy IoT applications for smart homes and industrial monitoring.
...
> `#Android` `#Instant Messaging` `#Netty` `#Protobuf` `#aio` `#bio` `#chat` `#heartbeat` `#im` `#netty` `#nio` `#offline-message` `#protobuf` `#reconnect-socket` `#socket` `#tcp` 
> NettyChat is an Android IM library built on Netty, TCP, and Protobuf, offering a complete instant messaging solution. It efficiently handles TCP packet fragmentation and sticking, and implements long-connection handshake authentication, heartbeat mechanism, reconnection, message resend, read/write timeout, offline messages, and a built-in thread pool. Ideal for Android IM apps requiring stable long connections, it can be directly integrated or customized.
...
> `#Distributed Services` `#Java` `#NIO` `#RPC Framework` 
> Hasting is a lightweight distributed RPC framework built on Java native I/O (NIO/OIO/AIO) and thread pools. It provides service registration/discovery, load balancing, high availability, generic invocation, rate limiting, RPC context, multiple registries (ZooKeeper, etcd, Redis), and a web console. Suitable for microservices architectures.
...
> `#Asynchronous I/O` `#Java NIO` `#Network Communication` `#http` `#java` `#java-nio` `#nio` `#protocol` `#websocket` 
> FireNio is an easy-to-use asynchronous I/O framework based on Java NIO and Linux epoll, designed to simplify the rapid development of network communication services. It offers a clean and concise API that enables developers to quickly build TCP, HTTP, and WebSocket servers with minimal effort. The framework comes with built-in automatic reconnection, heartbeat mechanisms, and SSL encryption (supporting both JdkSSL and OpenSSL). Additionally, it supports custom protocol extensions, including LengthValue and Protobase, for flexible data transmission. FireNio has demonstrated outstanding performance in TechEmpower Framework Benchmarks (TFB), making it an ideal choice for high-concurrency, low-latency server-side applications.
...
> `#Java NIO` `#Lightweight MVC` `#Web Server` `#http-server` `#nio` `#spring-mvc` 
> LightWebServer is a lightweight embedded web server built on Java NIO, compatible with HTTP/1.1, and includes a simple MVC framework. It uses annotations like @Controller and @WebPath for request mapping and parameter injection, supporting GET/POST, form data and file upload, suitable for building embedded or lightweight web applications quickly.
...
> `#Cache` `#Java` `#Local Cache` `#cache` `#caffeine` `#caffeine-cache` `#ehcache` `#guava-cache` `#hight-performance` `#memcached` `#redis` 
> A high-performance extensible local cache tool handcrafted in Java, progressively mimicking Redis features. It supports fluent API, fixed size, multiple eviction strategies (FIFO/LRU), expiration, persistence (RDB/AOF), and listeners, ideal for lightweight local caching scenarios.
...
> `#Distributed Caching` `#Distributed Systems` `#Java` 
> my-cache is a lightweight distributed cache implemented in pure Java. It supports standalone deployment, master-slave replication, key expiration, distributed locks, and persistence. With its well-designed client encapsulation, it can either run as an independent service for global use in distributed systems or be embedded as a local JAR dependency, making it ideal for small-to-medium projects to boost data access performance at low cost.
...
> `#Data Mocking` `#Java` `#Random Algorithms` `#Testing Tools` `#java-data` `#jmock` `#jmockdata` `#jmockit` `#mock-java-data` `#mocker` 
> Jmockdata is a powerful Java data mocking tool that uses random algorithms to simulate test data. It supports a wide variety of types, from primitives to complex objects, including collections, arrays, enums, and dates, and can generate data based on regular expressions. With advanced features like generics, inheritance, circular dependency handling, custom Mockers, and configurable strategies, Jmockdata is ideal for unit testing, API integration testing, and rapid prototyping, greatly improving the efficiency of test data creation and enhancing development workflows.
...
> `#Java NIO` `#Message Transfer` `#Network Communication` 
> A lightweight message transfer framework based on Java NIO, supporting text, binary, and mixed message transmission. It offers custom encryption algorithms, synchronous/asynchronous modes, built-in heartbeat monitoring, server authentication, and automatic reconnection after disconnection. Flexibly configurable for network communication scenarios.
...
> `#Java NIO` `#Networking Framework` `#RPC Development` 
> Mycat-NIO is a high-performance and simple Java NIO framework derived from the Mycat project, widely used for RPC development and foundational platforms. Its key highlight is SharedBufferPool, which uses a volatile variable for ByteBuffer allocation counting to avoid CAS overhead while ensuring visibility, boosting overall performance in high-concurrency networking.
...
> `#Asynchronous Networking` `#Java Framework` `#Web Server Framework` `#java` `#netty` `#nio` `#tls` `#voovan-web` `#webserver` `#websocket` 
> Voovan is a high-performance asynchronous networking and web server framework, featuring HTTP client, dynamic compilation, database access, hot deployment, and cluster support, aiming for elegant coding and high concurrency.
...
> `#Backend Framework` `#Java Middleware` `#Service Suite` `#bus` `#cache` `#cors` `#dubbo` `#i18n` `#limiter` `#logger` `#mapper` `#oauth` `#poi` `#proxy` `#sensitive` `#socket` `#spring` `#springboot` `#storage` `#swagger` `#tracer` `#vaildate` `#wrapper` 
> Bus is a Java-based backend foundation framework and service suite, inspired by many open-source designs. It offers clean code and clear architecture, suitable for building backend services and serving as middleware. Ideal for learning. The project is deprecated and moved to a new repository.
...
> `#Netty` `#Network Communication` `#Real-time Communication` `#Spring Boot` 
> hxy-socket is a high-performance socket communication framework based on Spring Boot and Netty, designed to achieve seamless integration between the two. It supports both WebSocket and TCP socket connections, provides built-in codecs for JSON and Protocol Buffers, and allows developers to extend custom protocols. The framework is annotation-driven, enabling quick and easy integration with Spring Boot applications. Additionally, it fully leverages the Spring ecosystem, making it convenient to use various Spring plugins and components. This makes hxy-socket ideal for scenarios such as instant messaging, real-time data push, IoT communication, and other high-concurrency networking applications.
...
> `#Async Client` `#Network Communication` `#RxJava` `#Socket` `#android` `#client` `#java` `#socket` `#socket-io` `#socket-io-client` `#socket-programming` `#socketio` `#sockets` `#tcp` `#tcp-client` `#tcp-server` 
> bizsocket is an asynchronous Socket client library designed for real-world business scenarios. It supports auto-reconnection, request/response, notification, sticky notification, serial request merging, packet fragmentation, caching, interceptors, and ships with RxJava and RxJava2 integrations, providing a Retrofit-like experience for Socket communication in Android/Java apps.
...
> `#Android Development` `#Request Callback` `#Socket Framework` `#TCP Long Connection` `#android` `#easysocket` `#socket` 
> EasySocket is a lightweight Android Socket framework enabling fast TCP long-connection communication between clients and servers. It supports various message protocols, chain-style API configuration, and unique request-response callback mapping. Built-in heartbeat, auto-reconnect, and packet fragmentation handling make it ideal for robust mobile network interactions.
...
> `#Android Development` `#Network Communication` `#Socket Wrapper` `#TCP/UDP` 
> XAndroidSocket is a lightweight Android Socket wrapper supporting TCP/UDP clients and servers. It provides built-in packet-sticking strategies (none, delimiter, fixed/variable length) and allows custom handling for packet assembly, validation (e.g., MD5), and decoding. With simple interfaces, it enables rapid integration of complex protocols for reliable network communication.
...
> `#Concurrency` `#Interview Prep` `#Java` `#Multithreading` `#concurrency` `#concurrent-programming` `#interview` `#interview-questions` `#java` 
> A comprehensive Java concurrency knowledge base covering fundamentals, JMM, synchronized/volatile, AQS, concurrent containers, thread pools, and atomic classes. It provides in-depth source code analysis and interview questions, ideal for systematic learning and interview preparation.
> <sub>concurrency</sub>
...
> `#Interview Prep` `#Java Ecosystem` `#技术教程` 
> Six-Finger is a comprehensive Java tutorial repository covering core topics from Java basics and JavaWeb to popular frameworks, interview questions, microservices, distributed systems, and big data. It helps learners build a complete knowledge system with well-organized articles and source code analysis, ideal for beginners and interview preparation.
...
> `#Computer Science Basics` `#Ebook Resources` `#Programming Learning` 
> This repository curates and shares download links for classic computer science textbooks, covering core areas such as math foundations and computer basics. Most resources are high-definition scanned versions with OCR and bookmarks for easy study. Available via permanent Tianyi Cloud links, it offers convenient reference materials for programmers' self-learning.
...
> `#Developer Resource` `#Spring Framework` `#Technical Documentation` 
> This project is a Chinese translation of the official Spring Framework 5.1.3 documentation, covering core modules such as dependency injection, transaction management, web applications, and data access. It offers multiple reading formats including GitHub and Gitbook, lowering the learning barrier for Chinese developers.
...
> `#DevOps Monitoring` `#OSHI` `#Server Monitoring` `#Spring Boot` `#agent` `#cpu` `#devops` `#grafana-dashboard` `#linux` `#memory` `#monitoring` `#monitoring-systems` `#nagios` `#prometheus` `#snmp` `#snmp-agent` `#springboot` `#springboot2` `#sysstat` `#webssh2` `#wgcloud` `#zabbix` 
> WGCLOUD is a lightweight distributed monitoring system based on SpringBoot and OSHI, covering CPU, memory, disk, GPU, Docker, database and more. It features dashboards, network topology, Web SSH, alerting, and supports Linux/Windows/Unix/macOS. Easy deployment.
...
> `#Feign` `#File Upload` `#REST Client` `#Spring` 
> This project demonstrates how to build a Feign-based REST client that supports multipart file uploads. By implementing a custom Encoder and reusing Spring's HttpMessageConverter chain to mimic RestTemplate, it overcomes Feign's native lack of multipart support. It handles single/multiple file uploads with optional JSON objects, offering a reference for extending Feign clients.
...
> `#Form Encoding` `#OpenFeign` `#微服务架构` `#feign` `#http` `#java` `#rest` 
> feign-form is a form encoder extension for OpenFeign, adding support for application/x-www-form-urlencoded and multipart/form-data. It integrates seamlessly with Feign.Builder, supports POJO parameter grouping, file and byte array uploads, and lets you decorate existing encoders like JacksonEncoder. Ideal for declarative HTTP clients, simplifying form submission and file transfer. Note: the repository is inactive and merged into the main feign project.
...
> `#Authentication` `#Multi-tenant` `#Spring Cloud` `#微服务架构` `#elk` `#gpe` `#java` `#jwt` `#microservices` `#nacos` `#oauth2` `#platform` `#rbac` `#sentiel` `#spring-boot` `#spring-cloud` `#spring-cloud-alibaba` `#zuul` 
> An enterprise-grade microservices multi-tenant architecture based on Spring Cloud Alibaba, featuring front-end/back-end separation and componentization for high cohesion and low coupling. It deeply customizes Spring Security to deliver stateless unified authentication via RBAC, JWT, and OAuth2, supports multi-tenant isolation, config center, distributed scheduling, log and slow-SQL monitoring, and is suitable for both learning and production.
...
> `#Gray Release` `#Service Governance` `#Spring Cloud` `#微服务架构` `#apollo` `#blue-green-deployment` `#cloud-native` `#consul` `#etcd` `#eureka` `#gray-release` `#hystrix` `#jaeger` `#nacos` `#opentelemetry` `#opentracing` `#redis` `#sentinel` `#skywalking` `#spring-cloud` `#spring-cloud-alibaba` `#spring-cloud-gateway` `#zookeeper` `#zuul` 
> Nepxion Discovery is a Spring Cloud-based cloud-native microservices solution offering blue-green and gray release, routing, rate limiting, circuit breaking, degradation, isolation, tracing, traffic dyeing, failover, and active-active capabilities, enabling robust service governance and traffic management.
...
> `#Offline Reading` `#PDF Document` `#Technical Interview` `#coding-interviews` 
> This repository provides PDF versions of the well-known CS-Notes technical interview notes, which cover a wide range of topics such as algorithms, operating systems, computer networking, Java, and more. The PDFs enable developers to read offline without an internet connection, making it convenient to review anytime. Additionally, the repository offers recommendations for programming books, interview preparation resources, and useful tools. Although the project is no longer actively maintained, the PDF content still holds significant reference value for those preparing for technical interviews and for continuous learning.
...
> `#Educational` `#Spring Cloud` `#微服务架构` 
> Classroom code for Spring Cloud Season 2 (2020.3) from Atguigu, covering core microservice components and practical examples, ideal for learning distributed architecture, service governance, and configuration management.
...
> `#Document Processing` `#LibreOffice` `#Online Editing` `#Watermark` `#jodconverter` `#libreoffice` `#watermark` 
> This system is an online document preview and editing platform built with LibreOffice and CKEditor. It converts Office documents such as DOC/DOCX, XLS, PPT into HTML for in-browser editing, and supports versioning with save as new version or discard changes. A built-in watermark processor handles both text and image watermarks across images, Word, Excel, PPT, and PDF, enabling copyright protection. Additionally, PDF files can be converted to HTML for online editing. This solution is ideal for enterprise knowledge bases, OA systems, and any scenario requiring secure online document collaboration.
...
> `#Design Patterns` `#Hands-on` `#Java` `#Spring` `#design-patterns` `#java` `#spring` 
> A Java-focused design patterns repository covering 12 classic patterns with explanations and hands-on projects. It demonstrates integrated use of Facade, Template Method, Chain of Responsibility, Strategy, and Factory patterns, plus Spring AOP dynamic proxies, ideal for developers to master design patterns in real-world scenarios.
